...
from .image_transforms import create_image_transform,create_simple_decoder, create_image_encoder, create_image_decoder, create_infoGAN_encoder, infoGAN_decoder
from .vector_transforms import create_vector_encoder, create_vector_transform, create_vector_decoder
from manifold_flow import transforms
from manifold_flow.flows import Flow, EncoderManifoldFlow, VariableDimensionManifoldFlow, ManifoldFlow,DenoisingManifoldFlow, ProbabilisticAutoEncoder


logger = logging.getLogger(__name__)


ALGORITHMS = ["flow", "pie","dnf","mf", "gamf", "hybrid", "emf", "pae","dnf_dec"]  # , "dough"
...
def create_model(args, simulator):
    ...
    elif args.algorithm in ["mf","dnf", "gamf", "pie"] and not args.specified and not simulator.is_image():
        model = create_vector_mf(args, simulator)

    # M-flow or PIE or DNF for image data
    ...
    # DNF with sep. decder for image data image data
    elif args.algorithm in ["dnf_dec"] and not args.specified and simulator.is_image():
        model = create_image_dnf_dec(args, c, h, simulator, w)
    ...
    # PAE for image data
    elif args.algorithm == "pae" and simulator.is_image():
        model = create_image_pae(args, c , h, simulator, w)
    else:
...
def create_image_dnf_dec(args, c, h, simulator, w):
    ...
    decoder = create_simple_decoder(c, w, h, args.dropout) 
    #infoGAN_decoder(args.modellatentdim,64,64,args.batchsize,dropout=0.2)
    #create_image_decoder(args.modellatentdim,channels_multiplier=15)
    model = DenoisingManifoldFlow(
...
def create_vector_mf(args, simulator):
    ...
    """
    inner_transform = transforms.ConditionalAffineScalarTransform(features=1)  #transforms.IdentityTransform() #
    """
    inner_transform = create_vector_transform(
        args.modellatentdim,
        args.innerlayers,
...
def create_image_pae(args,c,h,simulator,w):
    logger.info(
        "Creating PAE for image data with %s latent dimensions, encoder with %s blocks, decoder with %s blocks, %s inner layers, transform %s, %s context features",
        args.modellatentdim,
        args.encoderblocks,
        args.decoderblocks,
        args.innerlayers,
    ...
    encoder = create_infoGAN_encoder(c, h, w, latent_dim=args.modellatentdim,dropout=args.dropout, context_features=simulator.parameter_dim() if args.conditionalouter else None,)
    #create_image_encoder(3, 64, 64, latent_dim=args.modellatentdim, context_features=simulator.parameter_dim() if args.conditionalouter else None,)
    decoder = infoGAN_decoder(args.modellatentdim,64,64,args.batchsize,args.dropout) #todo:generalize for arbitrary spatial dimensions-->args
    #create_image_decoder(args.modellatentdim,channels_multiplier=15)
    inner_transform = create_vector_transform(
        args.modellatentdim,
        args.innerlayers,
