...
def create_image_encoder(
    ...
    encoder = nn_.ModifiedConvEncoder(h, w, channels_in=c, channels_multiplier=15, levels=4, out_features=latent_dim, activation=F.relu,)
    encoder = PreprocessingEncoder(encoder, preprocessing_transform)

    return encoder

def create_image_decoder(latent_dim,channels_multiplier):
    decoder = nn_.ConvDecoder(latent_features=latent_dim, channels_multiplier=channels_multiplier)
    return decoder

def create_simple_decoder(channels, width, height, dropout):
    decoder = nn_.simple_decoder(channels=channels, width=width, height=height, dropout=dropout)
    return decoder


def create_infoGAN_encoder(
    c, h, w, latent_dim, dropout, context_features=None, preprocessing="glow", alpha=0.05, num_bits=8
):
    assert context_features is None
    preprocessing_transform = _create_preprocessing(alpha, c, h, num_bits, preprocessing, w)
    encoder = nn_.infoGAN_encoder(latent_dim,dropout)
    encoder = PreprocessingEncoder(encoder, preprocessing_transform)
    return encoder

def infoGAN_decoder(latent_dim,width,height,batch_size,dropout):
    decoder = nn_.infoGAN_decoder(latent_dim,width,height,batch_size,dropout)
    return decoder
