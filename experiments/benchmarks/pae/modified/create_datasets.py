...
load_funcs=dict(banana=ld.load_banana,mnist=ld.load_mnist, fmnist=ld.load_fmnist, cifar10=ld.load_cifar10, celeba=ld.load_celeba, gan64=ld.load_gan64, gan2=ld.load_gan2)
    ...
    def train_input_fn():
        def mapping_function(x):
            def extract_images(inds):
                ...
                return xx          
            xx = tf.py_function(func=extract_images,inp=[x],Tout=tf.float32) #tion
            print('py_func xx',xx)
            xx.set_shape(shape)
            return xx
    ...
    def eval_input_fn():
        def mapping_function(x):
            def extract_images(inds):
                ...
                return xx/256.-0.5
            xx = tf.py_function(extract_images,[x],tf.float32) #tion 
            xx.set_shape(shape)
            return xx
