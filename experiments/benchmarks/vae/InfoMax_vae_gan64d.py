...
def train(VAE, D, optim_VAE, optim_D, vae_scheduler ,D_scheduler, data,vali, epochs=200):     
    val_best = 2**23
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if val_loss[1] / 40 < val_best:  #average val lass per batch
                ...
                torch.save(checkpoint , os.path.join(r'/storage/homefs/ch19g182/Python/vae/results', str(seed)+'checkpoints.pt'))
            VAE.train()
            D.train()
...
#PATH
output_dir = '/storage/homefs/ch19g182/Python/vae/results'
data_path = r'/storage/homefs/ch19g182/Python/dmanifold-flow-public/experiments/data/samples/gan64d/x_train.npy'   

##DATA
try:
    style = np.load(data_path)
except:
    print("Could not load data in path ",data_path)

style_train = torch.from_numpy(style[0:18000,:])
style_val = torch.from_numpy(style[18000:,:])

data_loader = DataLoader(dataset=style_train, batch_size=100, shuffle=True, num_workers=4) #, drop_last=True)
val_loader = DataLoader(dataset=style_val, batch_size=100, shuffle=True, num_workers=4) #, drop_last=True)
...
torch.save(checkpoint , os.path.join(output_dir, str(seed) + 'checkpoints_last.pt'))

print('training finished, have a nice day <3')