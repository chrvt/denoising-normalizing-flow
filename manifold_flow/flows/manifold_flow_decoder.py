...
class DenoisingManifoldFlow(BaseFlow):
    """ Manifold-based flow (base class for FOM, M-flow, PIE) """

    def __init__(self, data_dim, latent_dim, outer_transform,decoder=None, inner_transform=None, pie_epsilon=1.0e-2, apply_context_to_outer=True, clip_pie=False):
        super(DenoisingManifoldFlow, self).__init__()
        ...
        if decoder is None:
            self.decoder = transforms.IdentityTransform()
        else:
    ...
    def forward(self, x, mode="mf", context=None, return_hidden=False):
        ...
        assert mode in ["mf", "dnf","dnf_dec", "pie", "slice", "projection", "pie-inv", "mf-fixed-manifold"]
    ...
    def _decode(self, u, mode, u_orthogonal=None, context=None):
        ...
        if mode in ["pie", "slice", "projection", "mf-fixed-manifold", "dnf"]:
            ...
            h = self.projection.inverse(h)
            y, inv_log_det_outer = self.outer_transform.inverse(h, full_jacobian=False, context=context if self.apply_context_to_outer else None)
            x = self.decoder(y)
            inv_jacobian_outer = None
